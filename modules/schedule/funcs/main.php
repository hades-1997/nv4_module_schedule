...
if ($nv_Request->isset_request('checkss', 'post')) {
    ...
    */
	if (($fphone = $nv_Request->get_editor('fphone', '', NV_ALLOWED_HTML_TAGS)) == '') {
        nv_jsonOutput(array(
            'status' => 'error',
            'input' => 'fphone',
            'mess' => $lang_module['error_phone']
        ));
    }
	...
	if (($checkin = $nv_Request->get_editor('checkin', '', NV_ALLOWED_HTML_TAGS)) == '') {
        nv_jsonOutput(array(
            'status' => 'error',
            'input' => 'fcon',
            'mess' => $lang_module['error_checkin']
        ));
    }
	/*
	...
	*/
	...
	if( preg_match( '/^([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})$/', $nv_Request->get_string( 'checkin', 'post' ), $m ) )
	{
		$_hour = 0;
		$_min = 0;
		$checkin = mktime( $_hour, $_min, 0, $m[2], $m[1], $m[3] );
	}
	else
	{
		$checkin = 0;
    ...
    (cid, cat, title, content, checkin, send_time, sender_id, sender_name, sender_email, sender_phone, sender_address, sender_ip, is_read, is_reply) VALUES
    (' . $fpart . ', :cat, :title, :content, :checkin, ' . NV_CURRENTTIME . ', ' . $sender_id . ', :sender_name, :sender_email, :sender_phone, :sender_address, :sender_ip, 0, 0)';
    $data_insert = array();
    $data_insert['cat'] = $fcat;
    $data_insert['title'] = $ftitle;
    $data_insert['content'] = $fcon;
	$data_insert['checkin'] = $checkin;
    ...
    if ($row_id > 0) {
        $fcon_mail = contact_sendcontact($row_id, $fcat, $ftitle, $checkin, $fname, $femail, $fphone, $fcon, $fpart);
        ...
        if ($fsendcopy) {
            ...
            $fcon_mail = contact_sendcontact($row_id, $fcat, $ftitle, $checkin, $fname, $femail, $fphone, $fcon, $fpart, false);
